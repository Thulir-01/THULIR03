  ...
  return (
    <div className="flex h-full flex-col gap-3 overflow-hidden bg-surface-100 p-3">
      <PageHeader
        ...
        subtitle="Confirm completed results before they move to pathologist approval"
      />
        ...
        <div className={`flex min-h-0 flex-col gap-3 ${detail ? "w-[35%]" : "w-full"}`}>
          <div className="shrink-0 rounded-md border border-line-200 bg-surface-0 px-4 py-2.5 shadow-raised">
            <div className="relative flex-1">
              ...
              <input
                type="text"
                placeholder="Search orders awaiting verification…"
                value={search}
          ...
          <div className="min-h-0 flex-1 overflow-hidden rounded-md border border-line-200 bg-surface-0 shadow-raised">
            {loading ? (
              <LoadingState label="Loading verify queue…" rows={4} />
            ) : loadError ? (
              <ErrorState message={loadError} onRetry={() => load(search)} />
            ) : orders.length === 0 ? (
              <EmptyState
                icon={BadgeCheck}
                title={search ? "No orders match your search" : "No orders awaiting verification"}
                hint={search ? undefined : "Enter results first — completed orders appear here for confirmation."}
              />
            ) : (
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="shrink-0 text-right">
                          <span className="inline-flex items-center gap-1 text-[11px] font-semibold text-accent-700">
                            <CheckCircle2 className="size-3" /> {done}/{total} done
                          </span>
                        </div>
                      </div>
                    </button>
